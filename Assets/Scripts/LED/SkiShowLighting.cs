...
/// <summary>
/// Projecteur statique + 4 lyres RGBW sur LightingControllerIp / univers LightingUniverse.
...
namespace LedShow.LED
{
    ...
    public class SkiShowLighting : MonoBehaviour
    {
        ...
        [SerializeField] string controllerIp = "192.168.1.48";
        [SerializeField] int universe = LedNetworkConfig.LightingUniverse;
    ...
    float _keepAliveTimer;
        ...
        {
            if (!Application.isPlaying || !flashOnSpace || !isActiveAndEnabled)
                return;
        ...
        void SendUniverse()
        {
            if (!Application.isPlaying || _sender == null)
                return;
        ...
        /// <summary>
        /// Éteint lyres + projecteur (univers 33 sur .48). Appelable depuis le menu Blackout.
        /// </summary>
        public static void SendBlackout()
        {
            var black = new byte[ArtNetPacket.MaxDmxLength];
            var ip = LedNetworkConfig.LightingControllerIp;
            try
            {
                using (var sender = new ArtNetSender(ip))
                {
                    sender.SendUniverse((ushort)LedNetworkConfig.LightingUniverse, black, ArtNetPacket.MaxDmxLength);
                }
            }
            catch (Exception e)
            {
                Debug.LogWarning($"{nameof(SkiShowLighting)}: blackout u{LedNetworkConfig.LightingUniverse} échoué ({e.Message}).");
            }
        }
        ...
        void ConnectSender()
        {
            ...
            try
            {
                var ip = string.IsNullOrWhiteSpace(controllerIp)
                    ? LedNetworkConfig.LightingControllerIp
                    : controllerIp;
                _sender = new ArtNetSender(ip);
            }
            catch (Exception e)
            {
