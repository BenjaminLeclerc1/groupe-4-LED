...
namespace LedShow.LED
{
    ...
    public class SkiDescentGame : MonoBehaviour
    {
        ...
        [Tooltip("Si décoché : pas de game over au contact des obstacles (utile pour tester la timeline).")]
        ...
        const int PromptMarginFromBottom = 20;
            ...
            void OnEnable()
            {
                ...
                else if (!_editorPreviewActive && !Application.isPlaying)
                {
                    // Hors Play : afficher l'accueil sans créer le lighting (évite DMX en édition).
                    ShowHomeScreen();
                }
            }
        ...
        public void StartGame()
        {
            ...
            // Si une preview éditeur était restée active, elle bloquait les collisions.
        ...
        void Update()
        {
            ...
            if (_isGameOver)
            {
                ...
                var blinkPhase = Mathf.FloorToInt(_blinkTimer / BlinkInterval);
                if (blinkPhase != _lastHomeBlinkPhase)
                {
                    _lastHomeBlinkPhase = blinkPhase;
                    RenderDeathScreen();
                }
        ...
        void UpdateHomeScreen()
        {
            ...
            _blinkTimer += Time.deltaTime;
            // Ne redessine que quand le clignotement change (évite un re-upload
            // Art-Net / preview à chaque frame = effet "écran qui clignote").
            var blinkPhase = Mathf.FloorToInt(_blinkTimer / BlinkInterval);
            if (blinkPhase != _lastHomeBlinkPhase)
            {
                _lastHomeBlinkPhase = blinkPhase;
        ...
        {
            if (!collisionsEnabled || recordObstaclesOnJump || !_skier.HasPixels)
                return;

            // Preview éditeur hors Play uniquement : pas de game over en scrubbant.
            ...
            foreach (var obstacle in _obstacles)
            {
                ...
                if (IsAirObstacle(obstacle.Type, obstacle.Lane))
                {
                    // Piaf / vol : collision AABB complète (X et Y).
                    if (playerTop < obstacleBottom || playerBottom > obstacleTop)
                        continue;
                }
                else
                {
                    // Sol : on peut passer au-dessus (saut).
        ...
        void RenderFrame()
        {
            ...
            if (recordObstaclesOnJump)
                ...
                DrawNoCollisionHud(buffer);
            buffer.Apply();
            _wall.UpdatePreviews();
        }

        void DrawNoCollisionHud(LEDWallBuffer buffer)
        {
            const string label = "NO HIT";
            const int scale = 1;
            ...
            PixelFont.Draw(label, originX, originY, new Color(0.4f, 0.85f, 1f), scale, buffer.SetPixel);
        }
        ...
        public bool UseLevelTimeline
        {
            get => useLevelTimeline;
            set => useLevelTimeline = value;
        }

        public SkiLevelTimeline LevelTimeline
        {
            get => levelTimeline;
            set => levelTimeline = value;
        }
        ...
        {
            // Preview éditeur uniquement hors Play (évite le conflit avec la boucle jeu).
            if (Application.isPlaying)
                return;
